...
```{r configure, echo = F}
opts_chunk$set(fig.width=5, fig.height=5, dpi = 42 * 5)
```
...
## Licenses
Most datasets (`r sum(missouri$public.domain == 'Yes')`) on the
Missouri portal are listed as in the public domain.

```{r publicdomain, echo = FALSE}
print(p0)
```
...
```{r pdf_trafficsurvey, echo = FALSE}
print(p2)
```
...
```{r kinds_counts, echo = FALSE, fig.width = 10}
print(p5)
```
...
```{r kinds_prop, echo = FALSE, fig.width = 10}
print(p6)
```
...
```{r says_missouri, echo = FALSE, fig.width = 10}
print(p8)
```
