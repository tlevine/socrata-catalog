...
```{r raleigh}
sqldf("select title, created, format, 'https://' || portal || '/d/' || identifier as url from catalog where portal like '%ralei%' order by created limit 10;", dbname = '/tmp/catalog.db')
```
...
```{r sf-ten-february}
sqldf("select title, created, format, 'https://' || portal || '/d/' || identifier as url from catalog where portal = 'data.sfgov.org' and created >= '2012-01-01' and created < '2012-02-01' order by created limit 10;", dbname = '/tmp/catalog.db')
```

It looks like January is mostly externally linked, zipped shapefiles. Half of the
datasets say "shapefile" in their titles or descriptions.

```{r sf-shapefile}
p.sf.changes.shapefile
```

And a lot of the rest look like zipped shapefiles, even though the titles and
descriptions don't say so.

What about November and December?

### Bursts of dataset upload
Some of the plots make it look like lots of datasets were suddenly uploaded one day
